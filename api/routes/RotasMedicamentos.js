...
class RotasMedicamentos {
  static async novoMedicamento(req, res) {
    try {
      ...
      const medicamento = await BD.query(
  ...
  static async listarMedicamentos(req, res) {
    try {
      const medicamentos = await BD.query(
  ...
  static async listarMedicamentosPorUsuario(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Erro ao listar os medicamentos do usuário" });
    }
  }
  ...
  static async DeletarMedicamentos(req, res) {
    ...
    try {
      const medicamento = await BD.query(
        `UPDATE Medicamentos set ativo = false where id_medicamento = $1 RETURNING *`,
        [id_medicamento]
      );
  ...
  static async AtualizarTodos(req, res) {
    ...
    try {
      const medicamento = await BD.query(
        `UPDATE Medicamentos SET nome = $1, observacoes = $2, dosagem = $3, frequencia = $4, data_inicio = $5, data_fim = $6, horarios = $7, ativo = $8 WHERE id_medicamento = $9 RETURNING *`,
        [
          nome,
          observacoes,
          dosagem,
  ...
  static async Atualizar(req, res) {
    ...
    try {
      ...
      const query = `UPDATE Medicamentos SET ${campos.join(
        ", "
      )} WHERE id_medicamento = ${id_medicamento} RETURNING *`;
  ...
  static async buscarDados(req, res) {
    try {
      ...
      const queryMedicamentos = `
                SELECT * from medicamentos WHERE id_usuario = ${id_usuario} AND ativo = true
            `;

      const queryConsultas = `
                SELECT * FROM consultas WHERE id_usuario = ${id_usuario} AND ativo = true
            `;
